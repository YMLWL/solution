...
# Base cookies that don't require dynamic values
base_cookies = {
...
def get_cookies_with_location(kota: str, kecamatan: str, lat: float, lon: float) -> Dict[str, str]:
    """Get cookies with location information"""
    cookies = base_cookies.copy()
    cookies["gf_chosen_loc"] = f"%7B%22locality%22%3A%22{kecamatan}%22%2C%22name%22%3A%22{kecamatan.replace('-', ' ').title()}%2C%20{kota.title()}%22%2C%22serviceArea%22%3A%22{kota}%22%2C%22serviceAreaId%22%3A%221%22%2C%22latitude%22%3A{lat}%2C%22longitude%22%3A{lon}%2C%22category%22%3A%22%22%2C%22timezone%22%3A%22Asia%2FJakarta%22%2C%22found%22%3Atrue%7D"
    return cookies
...
def search_restaurant_near_me(kota: str, kecamatan: str, lat: float = -6.2250138, lon: float = 106.9004472) -> dict:
    """
    ...
    Args:
        ...
        lat(float): latitude coordinate, defaults to Jakarta
        lon(float): longitude coordinate, defaults to Jakarta
    ...
    response = requests.get(
        ...
        params=params,
        cookies=get_cookies_with_location(kota, kecamatan, lat, lon),
        headers=headers,
    )
    print(response)
...
def get_restaurant_details(kota: str, restaurant_name: str, id_restaurant: str, lat: float = -6.2250138, lon: float = 106.9004472) -> dict:
    """
    ...
    response = requests.get(
        url,
        cookies=get_cookies_with_location(kota, restaurant_name, lat, lon),
        headers=headers,
    )
...
def get_merchant_promo(id_restaurant: str, kota: str = "jakarta", lat: float = -6.2250138, lon: float = 106.9004472) -> dict:
    """
    ...
    """
    url = f"{base_url_gofood}/deals/profile?cart_amount=0&merchant_id={id_restaurant}&service_type=5&offeringToken=&distance=130&gf_plus_two_exp_enabled=true&tenant=gofood&picked_loc={lat},{lon}&fulfillment_modes=regular&allowed_payment_types=0,4,15,16"

    response = requests.get(
        url,
        cookies=get_cookies_with_location(kota, kota, lat, lon),
        headers=headers,
    )
...
def search_restaurant_by_food_name(food_name: str, kota: str = "jakarta", lat: float = -6.2250138, lon: float = 106.9004472) -> dict:
    """
...
def get_restaurant_review(kota: str, restaurant_name: str, restaurant_id: str, lat: float = -6.2250138, lon: float = 106.9004472) -> dict:
    """

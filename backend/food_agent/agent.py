...
class FoodAgent(Agent):
    def __init__(self):
        ...
        tools = [
            Tool(
                name="search_restaurant_near_me",
                description="Search for restaurants near the user's location",
                function=utils.search_restaurant_near_me
            ),
            Tool(
                name="get_restaurant_details",
                description="Get detailed information about a specific restaurant",
                function=utils.get_restaurant_details
            ),
            Tool(
                name="get_merchant_promo",
                description="Get available promotions for a merchant",
                function=utils.get_merchant_promo
            ),
            Tool(
                name="search_restaurant_by_food_name",
                description="Search restaurants by food name",
                function=utils.search_restaurant_by_food_name
            ),
            Tool(
                name="get_restaurant_review",
                description="Get reviews for a specific restaurant",
                function=utils.get_restaurant_review
            ),
            Tool(
                name="check_json",
    ...
    def process_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "role": "assistant",
                "parts": [{
                    "text": f"Mohon maaf, terjadi kesalahan sistem: {str(e)}"
                }]
            }
    ...
    def handle_error(self, error: Exception) -> Dict[str, Any]:
        ...
        if isinstance(error, requests.exceptions.RequestException):
            ...
            error_msg += "format data tidak valid. Tim kami sedang memperbaiki masalah ini."
        else:
